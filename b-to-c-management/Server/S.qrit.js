...
var hotel_issues = anyDB.createConnection('mysql://sqluser:sqlpass@example.com/db_issues',function(e,r){console.log('hotel.sqlerr: '+e);});
var nodemailer = require("../public_html/node_modules/nodemailer");
var gmail=nodemailer.createTransport("SMTP",{service:'Gmail','auth':{'user':'example@gmail.com','pass':'x'}});
...
socket.on('browse_allqr',function(_){
  ...
  var qrcode = qr('example.com/b.php?='+n12+'='+_.uid);//console.log(_.room);
...
socket.on('set_qr',function(_){
  console.log('example.com/b.php?=000000000000='+_.uid);
  var qrcode = qr('example.com/b.php?=000000000000='+_.uid);//console.log(_.room);
  var cells = qrcode.modules;//console.log(cells.length);
  var num=4;
  var qrsize=cells.length*num;
  var Canvas = require('../public_html/node_modules/canvas');
  ...
  scanipedia_sql.query("SELECT * FROM login WHERE (email='"+_.remail+"' OR name='"+_.rname+"')").on('row',function(r){
    ...
    if(result==='not_set'){
      ...
      sendgrid.send({
                ...
                html    : '<div id="header"style="width:316px;height:46px;background:#EAEEF2;border:2px solid #B1C5D9;margin-bottom:0px;margin-left:auto;margin-right:auto;text-align:center;"><img src="https://example.com/R/scanpedia.png"style="margin:10px;"/></div>'+
                          ...
                          'Click this <b><a href="https://example.com/qrl2.php?ref='+ref+'">link</a></b> to complete your registration.'+
                          '<br><br>'+
                          '<b>Have a nice day!</b>'+
                          '<br><br>'+
                          'The Scanpedia Team'+
...
socket.on('make_qr',function(_){
  var qrcode = qr('example.com/'+_.room);//console.log(_.room);
  var cells = qrcode.modules;//console.log(cells.length);
  var num=3;
  var qrsize=cells.length*num;
  var Canvas = require('../public_html/node_modules/canvas');
            ...
            for(var i=0;i<arrimgs.length;i++){
              ...
              sendgrid.send({
                ...
                html    : '<div class="HEAD"style="position:relative;height:91px;width:100%;background:#FFF;">'+
                          ...
                          '<img src="http://example.com/R/hsdemo.png"style="margin:5px;height:41px;outline:none;"href="http://www.hostingstudio.net"/>'+
                          '<img src="http://example.com/R/qritlogo41.png"style="height:41px;float:right;margin:5px;outline:none;"href="http://example.com"/>'+
                          '</div>'+
...
var adminemail='example.com@gmail.com';
...
s[1].WHEN['thailand_admin.security']=function(_){//socket.on('thailand_admin.security',function(_){
	...
	if(sqlvar){sqlvar.query("SELECT * FROM "+co/*_.c*/+" WHERE room='0'").on('row',function(res){r=res;}).on('end',function(){
		...
		if(r.user===_.user){
			...
			console.log('.................................................'+adminemail+' co='+co);
			//
			qrcode = qr('https://example.com/a.php?0&'+cid);
	...
	sqlvar.query("SELECT email FROM "+co/*_.c*/+" WHERE room='"+room+"'").on('row',function(r){console.dir(r);
		...
		/*else if(r.email.search('@hotmail')!==-1||r.email.search('@live')!==-1||r.email.search('@outlook')!==-1){
			hotmail.sendMail({
        		subject:c+' update received...',
       			from:'example.com@hotmail.com',
        		to:r.email,
        		html: 'Click this <b><a href="http://example.com/'+c+'-room.php?qr='+room+'">link</a></b> to veiw your room update.'
		...
		//else if(r.email.search('@gmail')!==-1||r.email.search('@')!==-1){
			gmail.sendMail({
        		...
        		from:'example.com@gmail.com',
        		to:r.email,
        		html: 'Click this <b><a href="http://example.com/'+co/*_.c*/+'-room.php?qr='+room+'">link</a></b> to veiw your room update.'
       			 },function(error,response){
        			if(error){
        				console.log(error+'!!!!!!!!!!!!');
...
s[1].WHEN['simple_qr:get']=function(_){//socket.on('simple_qr:get',function(_){
  ...
  qrcode = qr('https://example.com/r.php?'+_.n+'&'+cid);
  cells = qrcode.modules;
  num=5;
  qrsize=cells.length*num;
  Canvas = require('../public_html/node_modules/canvas');
