window.nonce="AYpc1yzBmXd7fwer7Qnd5Sk";
console.dir('script');
		...
		*/
		...
		,	standard:function(q,u,i,c,cb){
				...
				img.onload=function(){
					...
					var cells=qr('137.74.145.40/\/?'+go+'&'+u).modules;
		...
		,	basic:function(q,u){
				var go=QR.to1000000(q)
				,	cells=qr('137.74.145.40/\/?'+go+'&'+u).modules
				,	row
				,	num=4
				,	canvas=QR.canvas[0]
				,	ctx=canvas.getContext('2d');
		...
		$('#L').unbind().bind('click',function(){//console.log('L');
		    ...
		    */
			...
			var ⵌ={
				server2:'http://137.74.145.40:8004'//signals call
			...
			,	ȿ:{
					s:new WebSocket('ws://137.74.145.40:8005')//TODO fix if closed when next needed //see readyState
				,	fix:function(){ⵌ.ȿ.s=new WebSocket('ws://137.74.145.40:8005');return;}
			...
			,	AT:function(x,y){//mini block x,y
					...
					ⵌ.at=b.join();
					if([0,1].indexOf(Object.keys(ⵌ.ᨖ.syn).length)!==-1){
						console.log('only you');
						ⵌ.alone=setInterval(function(){
							if(ⵌ.ȿ.s.readyState==1){
								clearInterval(ⵌ.alone);
								ⵌ.VOID();
								}
							},1000);
						}//no peers only you
