var connect=function(host,sockets,ports,s,G,loader,app,env){
var wasbroken=false;
...
loader.show=function(){
		var elem=document.getElementById('timedout')
...
,	x:function(){//execute the app
		loader.log(loader.bar?(10):(wasbroken?'[re]':'')+'connecting');
		loader.log('Hello World!');
		if(typeof app==="function"){
			app(s,G,loader);
			}
		else{
			$.getScript('/REWIRE.temp/'+app,function(){
				init(s,G,loader);
				});}
		return;
		}
		...
		if(s[V].WHEN){
			if([-1,1].indexOf([s[m].uid+'.w1',s[m].uid+'.w'+V,s[m].uid].indexOf(U))){
				...
				if(f&&f!='ɦ'){
					...
					if(s[1].syncd.gotscript&&s[1].goNULLd){s[1].goNULLd();}
					}
			...
			if(f=='ɦ'){
				if(d.h=='::socket-uid;;'){
					...
					if(!d.nonce){
						if(loader.bar){loader.log(loader.bar?(7):loader.t.innerHTML);}
						M.a(d.re);
						}
					else{
						app=d.nonce;
...
,	r:function(){//reconnect
		...
		M.t=setTimeout(function(){M.c();},2000+(g/1000));
		g=null;
		return;
		}
,	c:function(){//connect
		loader.show();
		s[m]=s[m]||new WebSocket('ws://'+host+':'+(mp));//s[m]=s[m]||new WebSocket('ws://'+host+':'+(mp));
		s[m].sid=m;
		s[m].syncd={gotscript:false,nullready:false};
		s[m].onclose=function(){
			wasbroken=true;
			loader.log('Re-loading');//loader.log('s['+m+'] closed!');
...
,	w:function(l){//l=matchingServerList
		for(var i=0,v=l[i];i<l.length;++i,v=l[i]){
			if(!w[i]){
				...
				s[v]=s[v]||new WebSocket('ws://'+host+':'+(wp[i]));//s[v]=s[v]||new WebSocket('ws://'+host+':'+(wp[i]));
				s[v].sid=v;
