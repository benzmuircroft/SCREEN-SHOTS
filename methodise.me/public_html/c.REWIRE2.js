var connect=function(host,sockets,ports,s,G,loader,app){
var wasbroken=false;
...
loader.log=function(t){
	if(loader.bar){
		if(typeof t=='number'){
			loader.t.style.width=((loader.bar.max/15)*t)+'px';
			}
		else if(loader.text){
			loader.t.style.width='100%';
			loader.t.innerHTML=t;
			}}
	return;
	};
loader.done=function(){loader.$.style.display='none';loader.log('Loading...');return;};
var glass='<div id="GLASS"style="width:100%;height:100%;position:fixed;top:0px;left:0px;z-index:9999998;">'+'<div style="position:absolute;top:50%;left:50%;margin-top:-50px;margin-left:-100px;width:187px;height:65px;font-family:arial;color:#747474;font-size:10px;text-align:center;background:#E5E5E5;border:3px solid #BFBFBF;padding:10px;-webkit-border-radius: 5px;-moz-border-radius:5px;border-radius:5px;">'+'<b>Retrying Your Connection...<b><br><br><b><b id="RECONNECT"style="color:#747474;">0</b><br><br><b style="font-size:8px;color:#000;"><b style="color:#FF0000;">RE</b>WIRE.io</b></b>'+'</div></div>'
,	MEM={}
,	w=sockets.slice(1)
,	m=sockets[0]
,	wp=ports.slice(1)
...
,	x:function(){//execute the app
		//loader.log(loader.bar?(15):(wasbroken?'[re]':'')+'connecting');
...
,	a:function(){//attach api
		for(var i=0,v=sockets[i];i<sockets.length;++i,v=sockets[i]){
			loader.log(loader.bar?(9):'Attaching [RE]WIRE to socket '+v);
		...
		loader.log(loader.bar?(10):'[RE]WIRE API Ready');
...
,	q:function(){//query all ready
		...
		if(ready){
			loader.log(loader.bar?(7):'ready:'+ready);
			ready=null;
		...
		if(s[V].WHEN){
			if([-1,1].indexOf([s[m].uid+'.w1',s[m].uid+'.w'+V,s[m].uid].indexOf(U))){
				...
				if(f&&f!='ɦ'){
					...
					else{if(f=='null'){s[V].racelost=true;}"console.log('s['+V+']NO EVENT: '+f)";"console.dir(d)";}//debug ERR1
    				}
				else if(d.h=='::null-loaded;;'){
					s[m].syncd.nullready=true;
					if(s[0].syncd.gotscript&&s[0].goNULLd){s[0].goNULLd();}
					}
			...
			if(f=='ɦ'){
				if(d.h=='::socket-uid;;'){
					...
					loader.log(loader.bar?(6):d.h);
					M.q();
					}
				else if(d.h=='::dummy-workers;;'){//get the workers
					loader.log(loader.bar?(4):d.h);
					if(w[0]){M.w(d.l.split(','));}
					else{M.q();}
					}
				else if(d.h=='::api-rewire;;'){//server has full api and is loading app. go do the same! (re is a clue (did we just crash?))
					if(!d.nonce){
						if(loader.bar){loader.log(loader.bar?(8):loader.t.innerHTML);}
						M.a(d.re);
						}
					else{
						app=d.nonce;
...
,	r:function(){//reconnect
		...
		g=Number(document.getElementById('RECONNECT').innerHTML)+1;
		M.t=setTimeout(function(g){
			document.getElementById('RECONNECT').innerHTML=g;
			M.c();
		},1000*Number(g),g);
		g=null;
		return;
...
,	c:function(){//connect
		loader.show();
		s[m]=s[m]||new WebSocket('wss://'+host+':'+(mp));
		s[m].sid=m;
		s[m].syncd={gotscript:false,nullready:false};
		...
		s[m].onopen=function(){
			if(wasbroken){location.reload();}
			else{
				loader.log(loader.bar?(1):'s['+m+'] open');
...
,	w:function(l){//l=matchingServerList
		for(var i=0,v=l[i];i<l.length;++i,v=l[i]){
			if(!w[i]){
				...
				s[v]=s[v]||new WebSocket('wss://'+host+':'+(wp[i]));
				s[v].sid=v;
				...
				s[v].onopen=function(event){
					loader.log(loader.bar?(5):'s['+this.sid+'] open');
					...
					if(this.readyState==1){this.send(s[m].uid);}else{console.log(e);}
					};}}l=i=v=null;return;}
	};'> onconnect >';M.c();
};//connect
