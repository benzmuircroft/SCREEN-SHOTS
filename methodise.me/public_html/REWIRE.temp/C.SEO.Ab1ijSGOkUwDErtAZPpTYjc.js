window.nonce="Ab1ijSGOkUwDErtAZPpTYjc";
							...
							$('#tweet0').one('click',function(){
								...
								check=function(){
								    if(child.closed){
								        ...
								        clearInterval(timer);
										s[0][NULL].WIRE('usr:retweeter_check_retweeted:svr',{retweetedlink:d.tweets[0].tweetlink,row:row});
								    	}}
								clearInterval(timer);
								child=window.open('https://twitter.com/intent/retweet?tweet_id='+d.tweets[0].tweetlink,'','toolbar=0,status=0,width=600,height=400');
							...
							$('#tweet1').one('click',function(){
								...
								check=function(){
								    if(child.closed){
								        ...
								        clearInterval(timer);
										s[0][NULL].WIRE('usr:retweeter_check_retweeted:svr',{retweetedlink:d.tweets[1].tweetlink,row:row});
								    	}}
								clearInterval(timer);
								child=window.open('https://twitter.com/intent/retweet?tweet_id='+d.tweets[1].tweetlink,'','toolbar=0,status=0,width=600,height=400');
								timer=setInterval(check,500);
								});}
						if(d.tweets[2]){
							$('#row2').show();
							...
							$('#tweet2').one('click',function(){
								...
								check=function(){
								    if(child.closed){
								        ...
								        clearInterval(timer);
										s[0][NULL].WIRE('usr:retweeter_check_retweeted:svr',{retweetedlink:d.tweets[2].tweetlink,row:row});
								    	}}
								clearInterval(timer);
								child=window.open('https://twitter.com/intent/retweet?tweet_id='+d.tweets[2].tweetlink,'','toolbar=0,status=0,width=600,height=400');
								timer=setInterval(check,500);
								});}}
